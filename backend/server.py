...
async def security_alarm(event_type: str, details: dict, request=None, user=None, severity: str = "critical", notify: bool = True):
    ...
    if not notify or not SECURITY_ADMIN_PHONE:
        return
    try:
        # SMS sağanağını önle: aynı olay tipi için N dakikada en fazla 1 SMS
...
async def register_failure(key: str, request=None, max_fail: int = 5, lock_minutes: int = 15, alarm_event: str = None, user=None, details: dict = None) -> bool:
    ...
    # Pencere dolduysa sayaç sıfırdan başlar
...
async def create_session(user_id: str, token: Optional[str] = None, request=None, twofa: bool = False) -> str:
    ...
    if is_admin and not twofa:
        # SIKI KURAL: yönetici oturumu SMS doğrulaması olmadan hiçbir yoldan açılamaz
...
async def get_current_user(authorization: Optional[str] = Header(None), request: Request = None):
    ...
    if user.get("role") in ("admin", "yonetici") and not session.get("twofa_verified"):
        # SIKI KURAL: SMS doğrulamasından geçmemiş yönetici oturumu geçersizdir
        await db.user_sessions.delete_one(_session_query(token))
        # GÜVENLİK ALARMI: doğrulamasız yönetici erişim girişimi → SMS
    ...
    # Yönetici oturumu için son görülme + IP takibi (arka planda, isteği yavaşlatmadan)
    ...
    # No-show kısıtlaması süresi dolduysa otomatik kaldır (yalnızca no_show kaynaklı)
...
async def _admin_session_track(session: dict, request=None):
    ...
    try:
        ...
        # IP değişikliği kontrolü (farklı subnet = şüpheli)
        if cur_ip != "unknown" and login_ip != "unknown" and cur_ip != login_ip:
            # Kaç farklı IP'den kullanıldığını say
            ...
            if ip_changes == 1:
                # İlk değişiklik: uyar, öldürme (mobil ağ geçişi olabilir)
                ...
                # Mobil kullanicilarda IP surekli degisir (hucresel ag) -> oturumu OLDURME.
                # Yalnizca belirli esiklerde bir kez uyari SMS'i gonder (SMS saganagi olmasin).
                user_doc = await db.users.find_one({"user_id": session.get("user_id")}, {"_id": 0})
                await security_alarm(
                    "admin_session_ip_change",
...
async def _admin_watchdog_loop():
    """Her 2 dakikada bir tüm yönetici oturumlarını tarar.
    - SMS doğrulamasız oturumlar → imha + alarm
    - Son 10 dk içinde 3+ farklı IP → imha + alarm
    """
    await asyncio.sleep(30)  # Başlangıçta 30 sn bekle (startup tamamlansın)
    while True:
        try:
            adm_ids = [u["user_id"] async for u in db.users.find(
                {"role": {"$in": ["admin", "yonetici"]}}, {"_id": 0, "user_id": 1}
            )]
            # 1) 2FA'sız yönetici oturumlarını bul ve imha et
...
# Sadece tedarikçi yetkisi olanlar (esnaf veya supplier rolü + supplier_group atanmış)
...
# Personel/esnaf/tedarikçi dahil sınırlı yetkili kullanıcılar (ürün düzenleme için)
...
# --- Tedarikçi yardımcıları (Faz 1) ---
# Roller: "esnaf" (eski) ve "supplier" (yeni) — ikisi de tedarikçi sayılır.
# Tedarikçi grubu kullanıcıda iki farklı alanda tutulabiliyor: supplier_group VEYA supplier_name.
...
def get_user_supplier_group(user: dict) -> Optional[str]:
    """Kullanıcının bağlı olduğu tedarikçi grubunu döndürür (supplier_group veya supplier_name)."""
    if not user:
        return None
...
# --- Kurye yardımcıları ---
# Rol: "kurye". Kurye bir PAZARA (courier_market = pazar adı) atanır ve
# yalnızca o pazardan verilen EVE SERVİS siparişlerini görür/yönetir.
COURIER_ROLES = ("kurye",)
...
#   2. teslim alınmama  -> 60 GÜN boyunca yalnızca ONLINE ödeme (kapıda/tezgah kapalı)
#   3. teslim alınmama  -> 180 GÜN boyunca yalnızca ONLINE ödeme
#   4. ve sonrası       -> 365 GÜN (1 yıl) boyunca yalnızca ONLINE ödeme
# Kademeli ve SÜRELİ sistem (yasal orantılılık). "Süresiz" ceza kaldırıldı.
# Ceza süresi dolunca otomatik kalkar.
NO_SHOW_RESTRICTION_DAYS = 60  # (geriye dönük uyumluluk; artık kademeli sabitler kullanılıyor)
...
def _evaluate_no_show_restriction(user: dict) -> dict:
    ...
    Döner: {
      "count": int, "online_only": bool, "indefinite": bool,
      "until": datetime|None, "message": str
    }
    """
    user = user or {}
...
async def _apply_no_show_penalty(user_id: str, order: Optional[dict] = None,
    ...
    """Bir siparişte 'Teslim Alınmadı' işaretlendiğinde no-show sayacını artırır ve
    KADEMELİ + SÜRELİ politikaya göre ödeme kısıtı uygular, SMS gönderir, log kaydeder.
    Politika: 1->uyarı, 2->60g, 3->180g, 4+->365g. Uygulanan cezayı açıklayan sözlük döner."""
    if not user_id:
    ...
    # CEZA MUAFİYETİ: penalty_exempt=True olan kullanıcılara ceza uygulanmaz.
    ...
    # Süresiz ceza artık kullanılmıyor: her seviyede süreli.
    order_id = (order or {}).get("tx_id") or (order or {}).get("order_id") if order else None

    if new_count <= 1:
        # Sadece uyarı; kısıt yok. Varsa eski süresiz bayrağını da temizle.
        ...
        # KRİTİK: Müşteri uygulaması kapıda ödemeyi is_restricted ile engelliyor.
        # No-show kaynaklı kısıtlamayı is_restricted olarak da işaretle (süre dolunca otomatik kalkar).
    ...
    # SMS gönder (uyarı veya kısıtlama)
    sms_sent = _send_no_show_sms(user, order, level, until)

    # LOG: SMS kaydı (içerik loglanmaz, sadece metadata)
...
async def _maybe_expire_no_show(user: dict) -> dict:
    """No-show kaynaklı kısıtlamanın süresi dolduysa otomatik kaldırır.
    Sadece restriction_source == 'no_show' olan kısıtlamalar için çalışır
    (elle konulan admin kısıtlamalarına dokunmaz). Süre dolunca 'kalktı' SMS'i gönderir.
    Güncellenmiş user dict döner."""
    try:
        if not user:
            return user
        ...
        until_cmp = until if getattr(until, "tzinfo", None) else until.replace(tzinfo=timezone.utc)
        if now_utc() <= until_cmp:
            return user  # hâlâ aktif
        # Süre dolmuş -> kısıtlamayı kaldır
        ...
        # "Kısıtlama kalktı" SMS'i (opsiyonel, bir kez)
        try:
